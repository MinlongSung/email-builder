import type { Coordinates, PointerEventHandlers, SensorProps } from '@/features/dnd/core/types';
import { getCoordinatesDelta, getEventCoordinates, hasExceededDistance } from '@/features/dnd/core/utils';
...
export class AbstractPointerSensor {
  ...
  constructor(
    ...
    if (activationConstraint.hold) {
      this.holdTimeoutId = setTimeout(this.handleStart, activationConstraint.hold);
      return;
    }
    ...
    if (this.rafId === null) {
      this.rafId = requestAnimationFrame(() => {
        ...
        if (!this.isDragging && this.props.options.activationConstraint) {
          if (tolerance !== undefined && hasExceededDistance(delta, tolerance))
            return this.handleCancel();
          if (distance !== undefined && hasExceededDistance(delta, distance))
