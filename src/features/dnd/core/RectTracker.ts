...
export class RectTracker {
  ...
  private droppableContainers: Map<
    string,
    DroppableContainer & { dirty: boolean }
  > = new Map();
  ...
  getDroppableContainers(): Record<string, DroppableContainer> {
    const snapshot: Record<string, DroppableContainer> = {};
  ...
  start({
    ...
    scrollables.forEach((scrollable) => {
      ...
      this.scrollableContainers.set(scrollable.id, {
        ...scrollable,
        rect: scrollable.element.getBoundingClientRect(),
      });
    });
  }
