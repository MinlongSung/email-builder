...
import { addTree } from "@/features/document/utils/tree/addTree";
...
export function duplicateTree(
  ...
  for (const block of Object.values(tree.blocks)) {
    ...
    // remap children
    clone.childrenIds = block.childrenIds.map(
  ...
  // 4. Insert duplicated tree into document
  addTree(document, duplicated, parentId, index);
