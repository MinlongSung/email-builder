...
import { Heading, levels, type Level } from "../../core/extensions/nodes/Heading";
...
import { levelToHeading, type TemplateConfig } from "$lib/template/types";
...
function extractExtensionConfig(config: Partial<TemplateConfig>, prop: string) {
  ...
  const headingValues: Partial<Record<Level, string>> = {};
  levels.forEach(level => {
    const heading = levelToHeading(level);
    const headingValue = (config[heading] as any)?.[prop];
    if (headingValue !== undefined) headingValues[level] = headingValue;
  });
  if (Object.keys(headingValues).length) extracted.heading = headingValues;
